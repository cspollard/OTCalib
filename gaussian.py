...
for lab in [str(x) for x in range(1)]:
  ...
  alldata = torch.rand((ndata,), device=device) * 2 - 1
  alldata = special.erfinv(alldata)
  alldatacpu = alldata.detach().cpu().squeeze()

  allmc = torch.sort(torch.rand((nmc,), device=device))[0] * 2 - 1
  allmccpu = allmc.detach().cpu().squeeze()
  truecpu = special.erfinv(allmccpu)
